--[[
-- 此文件实现以下功能
-- 获取当前全时间×年×月×日×时×分×秒  输入：fulltime 小鹤双拼：全时间
-- 获取当前日期×年×月×日  &  ×月×日  输入：date     小鹤双拼：日期
-- 获取当前时间×时×分×秒             输入：time    小鹤双拼：时间
-- 获取当前星期                      输入：week    小鹤双拼：星期
-- 获取当前全农历×年×月×日 &  ×月×日   输入：lunar  小鹤双拼：农历
--]]
...
-- ...
-- translators请在方案配置文件*.schema.yaml的engine\translators:下面添加滤镜引用：
-- - lua_translator@date_translator
-- - lua_translator@week_translator
-- filters请在方案配置文件*.schema.yaml的engine\filters:下面添加滤镜引用：
-- - lua_filter@lua_function1
-- - lua_filter@lua_function2
--]]
...
date14=nLDate1
date8=nLDate2
date9=nLYear.."年"..nLMonth.."月"..nLDay.."日（农历）"
date10=nLDate3
date11=nLDate4
---------------------------------------------------------------------------------
...
local function translator(input, seg)
   if (input == "lunar") then
      --yield(Candidate("date", seg.start, seg._end, date14, " "))
